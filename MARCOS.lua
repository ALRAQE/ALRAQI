...
if text and text:match("^ضع عدد الاعلُضاء (%d+)$") and SudoBot(msg) then
local Num = text:match("ضع عدد الاعبببضاء (%d+)$") 
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
status  = '\n🔘| تم الغاء كتمه عام من المجموعات'
texts = usertext..status
...
if text and text:match("^طرداا (%d+)$") and Mod(msg) then 
local userid = text:match("^اااالبطرد (%d+)$") 
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text == "تاكلا للكل" and Mod(msg) then
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text == ("تنزيل اااالكل") and msg.reply_to_message_id_ ~= 0 and Manager(msg) then
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text == "بوللت" or text == "الاااسطورة"  then  
local ASTORH_Msg = {
'ها عمري ♥',
'شعندك !؟ 👀',
'انت البوت',
...
if text == 'الاحصاائيات' or text == 'المجموعات' or text == 'المشتركين' then
if Sudo(msg) then 
...
if text == 'مسح المشتراكين' and is_devtaha(msg) then   
...
if text == "وش راياك بهاذا" or text == "وش رايك بهاذ" then
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"حبيب '..Namebot..' ♥"," نشبه وماعنده سالفه😂😶","كثير كلام😹","والنعم والله♥","كفوووو ♥"}
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
if text == "وش راياك بهاذي" or text == "وش رايك بهذي" then
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
...
if text and text:match("^وضع لقااب (.*)$") and msg.reply_to_message_id_ ~= 0 and Constructor(msg) then
local timsh = text:match("^وضااع لقب (.*)$")
function start_function(extra, result, success)
if msg.can_be_deleted_ == false then 
...
if text == 'تعطيااال اليوتيوب' and Constructor(msg) then  
send(msg.chat_id_,msg.id_,'\n• تم الامر بنجاح')  
database:set(bot_id.."dl_yt_dl"..msg.chat_id_,"close") 
return false  
end 
if text == 'تفعيل اليوتياااوب' and Constructor(msg) then  
send(msg.chat_id_,msg.id_,'\n• تم الامر بنجاح')  
...
if text == 'تفعيل اااالتنزيل' and Manager(msg) then   
database:del(bot_id..'dw:bot:api'..msg.chat_id_) 
Text = '\n🔰| *تم تفعيل التنزيل*' 
send(msg.chat_id_, msg.id_,Text) 
end
if text == 'تعطيل اااالتنزيل' and Manager(msg) then  
...
if text == 'تفعيل الااردود' and Manager(msg) then   
database:del(bot_id..'lock:reply'..msg.chat_id_)  
Text = '🔰| تم تفعيل الردود'
send(msg.chat_id_, msg.id_,Text) 
end
if text == 'تعطيل الردااااود' and Manager(msg) then  
